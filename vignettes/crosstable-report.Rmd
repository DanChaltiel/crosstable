...
![officer example](man/figures/officer-example.png)

You can also download this example [here](man/figures/vignette_officer.docx).

### Code

This code will produce the example above.

First, we create 3 crosstables using availables datasets, along with a ggplot. Then, we start a document (using `officer::read_docx()`), add some titles and paragraphs, incorporate our tables and our plot, along with legends, and add some page breaks. Note that the legends have a `bookmark` argument that can be referred to in the text for automatic numbering.
