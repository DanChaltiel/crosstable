...
First, since it uses the power of the `label` attribute, let's start by building a labelled dataset. For convenience, this dataset is already packed into `crosstable` so don't bother re-creating it.
...
```
In this vignette, I only printed the `mpg`, `cyl` and `disp` variables (the 11 first rows), but all other variables were also described. 
...
Fortunately, you can use the `cross_to_flextable` function to turn any `crosstable` into a HTML, ready-to-print table, which will be automatically displayed in the Viewer if your are using RStudio. 
...
Here, I use the `keep_id` argument to keep the variable name, but in practice you usually drop it for the label. See `vignette("crosstable-officer")` ([link](crosstable-officer.html)) for more on `cross_to_flextable` and on how to integrate crosstables with `officer` and `Rmarkdown`.
