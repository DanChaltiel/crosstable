...
#' @keywords internal
#' @noRd
cross_survival=function(data_x, data_y, showNA, total, label, surv_digits, times, followup, margin,
    ...
    if(is.null(data_y)){
        rtn=summarise_survival_single(data_x[[1]], times=times, followup=followup, digits=surv_digits)
    } else if(is.character.or.factor(data_y[[1]])){
        rtn=summarise_survival_by(data_x[[1]], data_y[[1]], times=times, followup=followup, digits=surv_digits, 
