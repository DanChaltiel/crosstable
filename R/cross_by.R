...
cross_by = function(data_x, data_y, funs, funs_arg, percent_pattern, total, percent_digits,
    ...
    if(!is.numeric(data_y[[1]]) && isTRUE(effect) && by_levels!=2){
        ...
        #TODO cli
        cli_warn(c(glue("Cannot calculate crosstable effects as there is not exactly 2 groups in `by`."),
    ...
    rtn_tbl = imap_dfr(data_x, ~{
        ...
        if(anyNA(.x) && "NA" %in% .x) {
            ...
            #TODO cli
            cli_warn(c(glue('Cannot describe column "{.y}" as it contains both `NA` (missing values) and "NA" (string)'),
    ...
    if(nrow(errors)>0){
        ...
        if(is.null(data_y)){
          #TODO cli
          cli_warn(glue("Cannot describe column{s} {errors_s}"),
                 class = "crosstable_wrong_col_class_warning")
        } else {
          #TODO cli
          cli_warn(glue("Cannot cross column{s} {errors_s} by column '{y}' ({yy})",
    ...
    if("effect" %in% names(rtn_tbl) && any(rtn_tbl$effect=="No effect?")){
        ...
        #TODO cli
        cli_warn(glue("Cannot calculate crosstable effects for variable{s} '{v}'"),
