...
```{r, include=FALSE}
flextable::save_as_image(ct1, "man/figures/ct2.png")
```
...
`mtcars2` is a dataset which has labels, so they are displayed (see [here](https://danchaltiel.github.io/crosstable/articles/crosstable.html#dataset-modified-mtcars) for how to add some). 

`crosstable` is returning a plain R object (`data.frame`), but using `as_flextable` allows to output a beautiful HTML table that can be exported to Word or web documents.
...
```{r, include=FALSE}
flextable::save_as_image(ct2, "man/figures/ct1.png")
```
